...
### Examples
```powershell
/build-feature budget-alerts              # Start fresh build (default autonomy)
/build-feature budget-alerts --autonomous # Maximum autonomy
/build-feature budget-alerts --cautious   # BLOCK on any issue
/build-feature budget-alerts --max-iterations=10
/build-feature budget-alerts --resume     # After human fix
/build-feature budget-alerts --dry-run    # Preview plan only
```
...
```
Human: /build-feature budget-alerts
...
2. `/test pre-merge` - Run full test suite
...
When building for Finance Tracker, follow:
- Direct Supabase queries against `finance` schema
...
ARGUMENTS: <feature-name> [options]
