...
### Examples
```powershell
/verify-done budget-alerts            # Full verification
/verify-done budget-alerts --quick    # Fast check (critical only)
/verify-done budget-alerts --single:F3  # Single criterion
/verify-done budget-alerts --auto-only  # For autonomous build loop
/verify-done budget-alerts --report     # Show last result
```
...
When verifying Finance Tracker features, pay attention to:
- Supabase finance schema queries (correct schema prefix)
- Tailwind v4 styling and responsive layout
- API route validation with Zod
- Dashboard data accuracy (aggregations, calculations)
- FIRE projection correctness (compound interest, tax rules)
