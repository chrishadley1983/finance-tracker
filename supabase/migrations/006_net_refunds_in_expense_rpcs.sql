-- =============================================================================
-- Migration 006: Net refunds within expense categories
--
-- Refunds (credits) in a known expense category now NET against that category's
-- spend (debits add, refund credits subtract), matching lib/reports/classify.ts.
-- A category can go negative in a period when refunds exceed spend (e.g. leftover
-- holiday FX converted back, filed under Holiday Travel). Income stays
-- positives-only; uncategorised rows still count debits only (a credit can't be
-- assumed a refund). get_income_by_category is unchanged.
-- =============================================================================

-- Spending by category: NET per known non-income category (no amount<0 filter).
...
-- Budget vs actual: per-category actual nets (income positives only; expense net).
...
LANGUAGE sql STABLE AS $function$
    ...
    SELECT
      ...
      SUM(
        CASE
          ...
          WHEN NOT c.is_income THEN -t.amount
          ELSE 0
        END
      ) AS actual_amount
    FROM finance.transactions t
  ...
  SELECT
    c.id AS category_id, c.name AS category_name, c.group_name, c.is_income,
...
-- Savings rate: expense_actual nets categorised expenses; uncategorised debits
-- only; income positives only; honours exclude_from_totals.
...
LANGUAGE sql STABLE AS $function$
  ...
  SELECT
    bt.income_budget, bt.expense_budget, at.income_actual, at.expense_actual,
